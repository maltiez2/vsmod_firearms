...
public class PowderBarrel : BlockContainer
{
}

public class PowderBarrelEntity : BlockEntityContainer
{
...
public class CrateProperties
{
    ...
    public CrateTypeProperties this[string type]
    {
        get
        {
            if (!Properties.TryGetValue(type, out CrateTypeProperties? props))
            {
                return Properties["*"];
            }
...
public class CollectibleBehaviorBoatableCrate : CollectibleBehaviorHeldBag, IAttachedInteractions
{
    ...
    public override void OnInteract(ItemSlot bagSlot, int slotIndex, Entity onEntity, EntityAgent byEntity, Vec3d hitPosition, EnumInteractMode mode, ref EnumHandling handled, Action onRequireSave)
    {
        ...
        if (take && ownSlot != null)
        {
            ...
            int quantity = bulk ? stack.StackSize : 1;
...
public class BlockCrate : BlockContainer, ITexPositionSource, IAttachableToEntity, IWearableShapeSupplier
{
    ...
    public void CollectTextures(ItemStack stack, Shape shape, string texturePrefixCode, Dictionary<string, CompositeTexture> intoDict)
    {
        ...
        {
            this.Textures.TryGetValue(type + "-" + key, out CompositeTexture? ctex);
            if (ctex != null)
            {
            ...
            {
                Textures.TryGetValue(key, out CompositeTexture? ctex2);
    ...
    Cuboidf[] closedCollBoxes = new Cuboidf[] { new(0.0625f, 0, 0.0625f, 0.9375f, 0.9375f, 0.9375f) };
    ...
    public MeshData GenMesh(ICoreClientAPI capi, ItemStack contentStack, string type, string label, string lidState, CompositeShape cshape, Vec3f rotation = null)
    {
        ...
        if (label != null && Props.Labels.TryGetValue(label, out LabelProps? labelProps))
        {
            MeshData meshLabel = GenLabelMesh(capi, label, tmpTextureSource[labelProps.Texture], false, rotation);
    ...
    {
        Props.Labels.TryGetValue(label, out LabelProps? labelProps);
        ...
        capi.Tesselator.TesselateShape("cratelabel", shape, out MeshData? meshLabel, this, rot);
    ...
    {
        return new BlockDropItemStack[] { new(handbookStack) };
    }
